...
class CreateSalesInvoice extends StatefulWidget {
  ...
  @override
  State<CreateSalesInvoice> createState() => _CreateSalesInvoiceState();
}

class _CreateSalesInvoiceState extends State<CreateSalesInvoice> {
  @override
  void initState() {
    context.read<HomeCubit>().productList.clear();
    context.read<HomeCubit>().productList.addAll(widget.item.salesInvoiceItems);
    super.initState();
  }
      ...
      body: CustomScrollView(
        slivers: [
          SliverAppBar(
            ...
            //    Navigator.push(context, MaterialPageRoute(builder: (context)=>SelectedItems(items: widget.item.salesInvoiceItems,) ));
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  TitleItem(title: 'اسم العميل'),

                  CustomTextField(
                    readOnly: true,
                    hintText: widget.item.customerName.toString(),
                    textvaidation: 'برجاء ادخال اسم العميل',
                  ),
                  TitleItem(title: 'رقم العميل'),

                  CustomTextField(
                    readOnly: true,
                    hintText: widget.item.customerPhone.toString(),
                    textvaidation: 'برجاء ادخال رقم العميل',
                  ),

                  TitleItem(title: 'عنوان العميل'),

                  CustomTextField(
                    readOnly: true,
                    maxlines: 3,
                    hintText: widget.item.orderAddress.toString(),
                  ...
                  Padding(
                    ...
                    child: BlocBuilder<HomeCubit, HomeState>(
                      builder: (context, state) {
                        if (state is AddEmployeeSalesInvoiceLoading ||
                            state is AddTransfereInvoiceLoading ||
                            state is AddSalesInvoiceWithoutTransfereLoading) {
                          return Center(child: CircularProgressIndicator());
                        }
                        return BlocListener<HomeCubit, HomeState>(
                          listener: (context, state) {
                            if(state is AddSalesInvoiceWithoutTransfereSuccess){
  ...
  void logic(BuildContext context, HomeCubit cubit) {
    if (context.read<HomeCubit>().productList.isNotEmpty) {
      if (cubit.selectedEmpId != null) {
        if (widget.orderType == OrderType.salesInvoice) {
          context.read<HomeCubit>().addEmployeeSalesInvoice(
            model: model(cubit, context),
          );
        } else if (widget.orderType == OrderType.transfereInvoice) {
          context.read<HomeCubit>().addTransfereInvoice(
            model: model(cubit, context),
          );
        } else if (widget.orderType == OrderType.salesInvoiceWithoutTransfere) {
          context.read<HomeCubit>().addSalesInvoiceWithoutTransfereLoading(
            model: model(cubit, context),
          );
        }
      } else {
        Fluttertoast.showToast(msg: 'يرجى اختيار المندوب');
      }
    } else {
      Fluttertoast.showToast(msg: 'يرجى اختيار الاصناف');
    }
  }
  ...
  AddEmployeeSalesInvoice model(HomeCubit cubit, BuildContext context) {
    return AddEmployeeSalesInvoice(
      ...
      employeeID: int.parse(cubit.selectedEmpId!),
      isCustomerPoint: false,
      tax: 0,
      totalDiscount: 0,
      totalVal: context.read<HomeCubit>().calculateTotal(),
      finalValue: context.read<HomeCubit>().calculateTotal(),
      totalQty: context.read<HomeCubit>().employeeList.length,
      totalExtra: 0,
      countryName: '',
      regionName: '',
      districtName: '',
      ...
      floor: '',
      apartment: '',
      orderAddress: cubit.addressController.text,
      ...
      extraDirections: '',
      passportID: '',
      salesInvoiceItems: context.read<HomeCubit>().productList,
      salesInvoicePayments: [
        SalesInvoicePayment(
          cardNo: '',
          paymentTypeID:
              widget.orderType == OrderType.salesInvoiceWithoutTransfere
                  ? 2
                  : cubit.paymenttype,
          approvalNo: removespaces(cubit.cardNumberController.text),
          value: context.read<HomeCubit>().calculateTotal(),
        ),
      ],
      coupons: [],
    );
